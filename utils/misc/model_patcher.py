...
class LowVramPatch:
    ...
    def __call__(self, weight):
        ...
        if intermediate_dtype not in [torch.float32, torch.float16, torch.bfloat16]: #intermediate_dtype has to be one that is supported in math ops
            ...
            return float.stochastic_rounding(lora.calculate_weight(self.patches[self.key], weight.to(intermediate_dtype), self.key, intermediate_dtype=intermediate_dtype), weight.dtype, seed=string_to_seed(self.key))
...
def get_key_weight(model, key):
    ...
    if len(op_keys) < 2:
        weight = utility.get_attr(model, key)
    else:
        op = utility.get_attr(model, op_keys[0])
        try:
...
class ModelPatcher:
    ...
    def get_model_object(self, name):
        if name in self.object_patches:
            ...
            if name in self.object_patches_backup:
                ...
                return utility.get_attr(self.model, name)
    ...
    def patch_weight_to_device(self, key, device_to=None, inplace_update=False):
        ...
        if set_func is None:
            out_weight = float.stochastic_rounding(out_weight, weight.dtype, seed=string_to_seed(key))
            if inplace_update:
                utility.copy_to_param(self.model, key, out_weight)
            else:
                utility.set_attr_param(self.model, key, out_weight)
        else:
    ...
    def patch_model(self, device_to=None, lowvram_model_memory=0, load_weights=True, force_patch_weights=False):
        with self.use_ejected():
            for k in self.object_patches:
                old = utility.set_attr(self.model, k, self.object_patches[k])
    ...
    def unpatch_model(self, device_to=None, unpatch_weights=True):
        ...
        if unpatch_weights:
            ...
            for k in keys:
                ...
                if bk.inplace_update:
                    utility.copy_to_param(self.model, k, bk.weight)
                else:
                    utility.set_attr_param(self.model, k, bk.weight)
        ...
        for k in keys:
            utility.set_attr(self.model, k, self.object_patches_backup[k])
                ...
                if hasattr(m, "comfy_patched_weights") and m.comfy_patched_weights == True:
                    ...
                    for param in params:
                        ...
                        if bk is not None:
                            ...
                            if bk.inplace_update:
                                utility.copy_to_param(self.model, key, bk.weight)
                            else:
                                utility.set_attr_param(self.model, key, bk.weight)
                            self.backup.pop(key)
    ...
    def calculate_weight(self, patches, weight, key, intermediate_dtype=torch.float32):
        print("WARNING the ModelPatcher.calculate_weight function is deprecated, please use: utils.misc.lora.calculate_weight instead")
    ...
    def patch_cached_hook_weights(self, cached_weights: dict, key: str, memory_counter: MemoryCounter):
        if key not in self.hook_backup:
            weight: torch.Tensor = utility.get_attr(self.model, key)
        ...
        utility.copy_to_param(self.model, key, cached_weights[key][0].to(device=cached_weights[key][1]))
    ...
    def unpatch_hooks(self) -> None:
        with self.use_ejected():
            ...
            for k in keys:
                utility.copy_to_param(self.model, k, self.hook_backup[k][0].to(device=self.hook_backup[k][1]))
