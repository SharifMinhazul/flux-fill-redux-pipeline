...
from utils.misc.k_diffusion import sampling as k_diffusion_sampling
from utils.misc.extra_samplers import uni_pc
from typing import TYPE_CHECKING
if TYPE_CHECKING:
    from utils.misc.model_patcher import ModelPatcher
    from utils.misc.model_base import BaseModel
    from utils.misc.controlnet import ControlBase
import torch
import collections
from utils.misc import model_management
import math
import logging
import utils.misc.samplers
import utils.misc.sampler_helpers
import utils.misc.model_patcher
import utils.misc.patcher_extension
import utils.misc.hooks
import scipy.stats
import numpy
...
def finalize_default_conds(model: 'BaseModel', hooked_to_run: dict[utils.misc.hooks.HookGroup,list[tuple[tuple,int]]], default_conds: list[list[dict]], x_in, timestep):
    ...
    for i, mult in enumerate(default_mults):
        ...
        for x in cond:
            ...
            p = utils.misc.samplers.get_area_and_mult(x, x_in, timestep)
            if p is None:
                continue
...
def calc_cond_batch(model: 'BaseModel', conds: list[list[dict]], x_in: torch.Tensor, timestep, model_options):
    executor = utils.misc.patcher_extension.WrapperExecutor.new_executor(
        _calc_cond_batch,
        utils.misc.patcher_extension.get_all_wrappers(utils.misc.patcher_extension.WrappersMP.CALC_COND_BATCH, model_options, is_model_options=True)
    )
...
def _calc_cond_batch(model: 'BaseModel', conds: list[list[dict]], x_in: torch.Tensor, timestep, model_options):
    ...
    hooked_to_run: dict[utils.misc.hooks.HookGroup,list[tuple[tuple,int]]] = {}
    default_conds = []
    ...
    for hooks, to_run in hooked_to_run.items():
        while len(to_run) > 0:
            ...
            if 'transformer_options' in model_options:
                transformer_options = utils.misc.patcher_extension.merge_nested_dicts(transformer_options,
...
def calc_cond_uncond_batch(model, cond, uncond, x_in, timestep, model_options): #TODO: remove
    logging.warning("WARNING: The utils.misc.samplers.calc_cond_uncond_batch function is deprecated please use the calc_cond_batch one instead.")
...
def resolve_areas_and_cond_masks(conditions, h, w, device):
    logging.warning("WARNING: The utils.misc.samplers.resolve_areas_and_cond_masks function is deprecated please use the resolve_areas_and_cond_masks_multidim one instead.")
...
def preprocess_conds_hooks(conds: dict[str, list[dict[str]]]):
    ...
    hook_replacement: dict[tuple[ControlBase, utils.misc.hooks.HookGroup], list[dict]] = {}
    for k in conds:
        for kk in conds[k]:
            if 'control' in kk:
                ...
                if len(extra_hooks) > 0:
                    hooks: utils.misc.hooks.HookGroup = kk.get('hooks', None)
    ...
    for key, conds_to_modify in hook_replacement.items():
        control = key[0]
        hooks = key[1]
        hooks = utils.misc.hooks.HookGroup.combine_all_hooks(control.get_extra_hooks() + [hooks])
...
class CFGGuider:
    ...
    def inner_set_conds(self, conds):
        for k in conds:
            self.original_conds[k] = utils.misc.sampler_helpers.convert_cond(conds[k])
    ...
    def inner_sample(self, noise, latent_image, device, sampler, sigmas, denoise_mask, callback, disable_pbar, seed):
        ...
        extra_args = {"model_options": utils.misc.model_patcher.create_model_options_clone(self.model_options), "seed": seed}

        executor = utils.misc.patcher_extension.WrapperExecutor.new_class_executor(
            sampler.sample,
            sampler,
            utils.misc.patcher_extension.get_all_wrappers(utils.misc.patcher_extension.WrappersMP.SAMPLER_SAMPLE, extra_args["model_options"], is_model_options=True)
        )
        samples = executor.execute(self, sigmas, extra_args, callback, noise, latent_image, denoise_mask, disable_pbar)
        return self.inner_model.process_latent_out(samples.to(torch.float32))

    def outer_sample(self, noise, latent_image, sampler, sigmas, denoise_mask=None, callback=None, disable_pbar=False, seed=None):
        self.inner_model, self.conds, self.loaded_models = utils.misc.sampler_helpers.prepare_sampling(self.model_patcher, noise.shape, self.conds)
        device = self.model_patcher.load_device

        if denoise_mask is not None:
            denoise_mask = utils.misc.sampler_helpers.prepare_mask(denoise_mask, noise.shape, device)
        ...
        utils.misc.sampler_helpers.cleanup_models(self.conds, self.loaded_models)
    ...
    def sample(self, noise, latent_image, sampler, sigmas, denoise_mask=None, callback=None, disable_pbar=False, seed=None):
        ...
        try:
            ...
            self.model_options = utils.misc.model_patcher.create_model_options_clone(self.model_options)
            # if one hook type (or just None), then don't bother caching weights for hooks (will never change after first step)
            orig_hook_mode = self.model_patcher.hook_mode
            if get_total_hook_groups_in_conds(self.conds) <= 1:
                self.model_patcher.hook_mode = utils.misc.hooks.EnumHookMode.MinVram
            utils.misc.sampler_helpers.prepare_model_patcher(self.model_patcher, self.conds, self.model_options)
            executor = utils.misc.patcher_extension.WrapperExecutor.new_class_executor(
                self.outer_sample,
                self,
                utils.misc.patcher_extension.get_all_wrappers(utils.misc.patcher_extension.WrappersMP.OUTER_SAMPLE, self.model_options, is_model_options=True)
            )
