...
def model_lora_keys_unet(model, key_map={}):
    ...
    diffusers_keys = utility.unet_to_diffusers(model.model_config.unet_config)
    ...
    if isinstance(model, model_base.SD3): #Diffusers lora SD3
        diffusers_keys = utility.mmdit_to_diffusers(model.model_config.unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    if isinstance(model, model_base.AuraFlow): #Diffusers lora AuraFlow
        diffusers_keys = utility.auraflow_to_diffusers(model.model_config.unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    if isinstance(model, model_base.Flux): #Diffusers lora Flux
        diffusers_keys = utility.flux_to_diffusers(model.model_config.unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
